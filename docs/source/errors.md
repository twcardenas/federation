...
| `SHAREABLE_WITH_MISMATCHED_RUNTIME_TYPES` | A shareable field return type have mismatched possible runtime types in the subgraphs in which the field is declared. As shared fields must resolve the same way in all subgraphs, this is almost surely a mistake. | 2.0.0 |  |
